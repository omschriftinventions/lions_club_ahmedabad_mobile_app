  ...
  return (
    <>
      <div className="page-head">
        ...
        {member?.canEdit && <button className="btn primary" onClick={() => setEditing({})}><Icon name="plus" size={16} /> Add award</button>}
      </div>
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                {member?.canEdit && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <button className="btn ghost sm" title="Edit" onClick={() => setEditing(a)}><Icon name="edit" size={15} /></button>
                    <button className="btn ghost sm" title="Delete" disabled={del.isPending}
                      onClick={() => { if (confirm('Delete this award?')) del.mutate(a.id); }}
                      style={{ color: 'var(--danger, #b3261e)' }}><Icon name="trash" size={15} /></button>
                  </div>
                )}
              </div>
      ...
      {editing && <AwardModal award={Object.keys(editing).length ? editing : null} members={members.data?.members ?? []} onClose={() => setEditing(null)} qc={qc} />}
    </>
  );
}

const AwardModal: React.FC<{ award: any | null; members: Member[]; onClose: () => void; qc: any }> = ({ award, members, onClose, qc }) => {
  const editing = !!award;
  ...
  return (
    <Modal title={editing ? 'Edit award' : 'Add award'} onClose={onClose}
      ...
      <div className="row-2">
        <Field label="Icon (emoji)"><input className="input" value={f.icon} onChange={set('icon')} maxLength={4} /></Field>
        <Field label="Category"><input className="input" value={f.category} onChange={set('category')} placeholder="e.g. Service" /></Field>
      </div>
      <Field label="Awarded to (optional)">
        <select className="select" value={f.member_id} onChange={set('member_id')}>
